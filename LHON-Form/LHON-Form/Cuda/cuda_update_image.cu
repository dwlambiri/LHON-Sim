...
extern "C" __global__  void cuda_update_image(unsigned short im_size, unsigned short bmp_im_size, float bmp_image_compression_ratio,
	unsigned char* bmp, float* tox, int offset, unsigned char* axon_mask, unsigned char* init_insult_mask, float tox_max, float tox_var, bool* show_opts)
{
	...
	if (x_bmp < bmp_im_size && y_bmp > 0) {
		...
		float tmp = tox[offset+xy] / ((1+tox_var*x_bmp/bmp_im_size)*tox_max);
		if (tmp > 1) tmp = 1;
