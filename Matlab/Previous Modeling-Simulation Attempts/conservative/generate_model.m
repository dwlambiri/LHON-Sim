...
bundle_dens = read_pair('bundle_dens', 1);
...
function Update(~,~)
    ...
    t4 = str2num(h_bundle_dens.String);
    ...
    M.nerve_r = nerve_r;
    axis(ax_ui); cla;
    plot_model('no_neuron', 'no_fig');
    h_feedback.String = sprintf('Model updated! (%d bundle(s))', size(bund_g, 2));
end

function h = pair_text(txt, pos, def)
    uicontrol('style','edit', 'Units','Normalized', 'position', [pos .12 .05], 'String', txt, 'Enable', 'off', 'BackgroundColor', [0.9400 0.9400 0.9400]);
    h = uicontrol('style','edit', 'Units','Normalized', 'position', [pos+[.12 0] .12 .05], 'String', num2str(def));
end
ok = 0;
    function OK(~,~)
        delete(fig_ui);
...
if any(f('GUI'))
    ...
    ax_ui = axes('units', 'normalized', 'position', [0.05 0.1 .7 .8]);
    
    h_feedback = pair_text('Status:', [.05 .92], []); h_feedback.Position = [.13 .92 .8 .05]; h_feedback.Enable = 'off';
    h_nerv_r = pair_text('Nerve Raduis:', [.75 .8], nerve_r);
    h_min_dis = pair_text('Min Clearance:', [.75 .7], min_bundles_dis);
    h_bundle_r_range = pair_text('Bundle Raduis Range:', [.75 .6], bundle_r_range);
    h_bundle_dens = pair_text('Bundle Density:', [.75 .5], bundle_dens);
    uicontrol('style','pushbutton', 'Units','Normalized', 'position', [.84 .4 .12 .05], 'String', 'Update', 'Callback', @Update);
    uicontrol('style','pushbutton', 'Units','Normalized', 'position', [.84 .3 .12 .05], 'String', 'Done/Continue', 'Callback', @OK);
    Update(0,0);
...
fprintf('\n');
    ...
    function M = create_csg(M, varargin)
        ...
        if isfield(M, 'csg') && ~any(ff('rewrite')), return, end
...
%         imagesc(M.r_dist.r_avg);
        surf(M.r_dist.xq, M.r_dist.yq, M.r_dist.r_avg/neuron_scale, 'edgecolor', 'none'); view(0, 90);
        axis equal
        colorbar
    end


    function fig = plot_model(varargin)
        ff = @(f) (cellfun(@(x) ischar(x) && strcmp(x,f), varargin));
        no_neuron = ff('no_neuron');
        no_fig = ff('no_fig');
        varargin([find(no_fig) find(no_neuron)]) = [];
        if any(no_fig)
            fig = [];
        else
            if nargout > 0
        ...
        axis equal, hold on
        draw_circle([0 0], M.nerve_r, 20, varargin{:});
    ...
    function plot_histogram(varargin)
        ...
        if isempty(varargin)
            ...
            for idx = 1:length(M.neuron)
                ...
                ax(idx) = axes();
                histogram(M.neuron{idx}(3,:), max(5, round(size(M.neuron{idx}, 2)/11)), opts{:});
