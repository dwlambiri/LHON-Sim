%str_res = Toxication_Boundary_2D('2016-11-22 @11-48-42 (Tolerance)(Neur Rate)', 'Test');
% str_res.TwoD_Toxic_Level('a');

function str = Toxication_Boundary_2D(varargin)
...
chron_prog_start = [];
    ...
    function n = get_sim_name(nam)
        % tkn = regexp(nam, '\(([\w.\s-]+)\)', 'tokens');
        tkn = regexp(nam, '([\W\w]+).prgim', 'tokens');
        n = tkn{1};
    end
