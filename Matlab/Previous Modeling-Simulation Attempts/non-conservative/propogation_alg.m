...
%% Play (and record) Animation
    function anim(varargin)
        ...
        dt = read_pair('dt', 0.2);
        show_cont = 1; % any(f('show_cont')); % show infection contour
        ...
        fig = figure('units', 'normalized', 'toolbar','figure', 'Name', 'Bundle Settings', 'outerposition', [0.2 0.15 .6 .8], 'WindowScrollWheelFcn', @wheel_CB, 'keypressfcn', @key_cb);
        function wheel_CB(~,e)
            sld.Value = sld.Value + e.VerticalScrollCount/100;
            sld.Value = max(min(sld.Value, 1), 0);
        end
        function key_cb(~,e)
